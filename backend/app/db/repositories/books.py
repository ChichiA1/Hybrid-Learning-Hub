...
class BookRepository:
    ...
    async def retrieve_book_quantity(db: AsyncSession, book_id: int) -> Book:
        try:
    ...
    async def add_book(db: AsyncSession, book: BookModel):
        try:
            # find out if book exist in the database and quantity there is
            book_in_bd = await BookRepository.retrieve_book_quantity(db,book.book_id)
            ...
            if  book_in_bd is None:
                ...
                book_in_bd.quantity += book.quantity
